...
start-candidate-client:
	cd apps/candidate-client && npm run dev

start-recruiter-client:
	cd apps/recruiter-client && npm run dev
...
generate-openapi:
	dotnet tool run swagger tofile --output ./docs/openapi.json ./apps/Server/Server.API/bin/Debug/net8.0/Server.API.dll v1